...
export class StreamableHttpClientTransport implements Transport {
  ...
  async openServerSentEventsListener(
    ...
    if (!this._sessionId) {
      throw new Error(
        "Cannot establish server-side listener without a session ID",
      );
    }
  ...
  async close(): Promise<void> {
    ...
    for (const reader of this._sseConnections.values()) {
